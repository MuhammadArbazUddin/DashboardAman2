...
const Sidebar = () => {
  ...
  return (
    ...
    >
      <div className="h-full bg-[#465048] p-4 flex flex-col border-r ">
        <div className="flex items-center">
          <motion.button
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className="p-2 rounded-full hover:bg-[#5d695e] transition-colors max-w-fit"
          >
            <Menu size={24} />
          </motion.button>
        </div>
        <nav className="mt-8 flex-grow">
          {SIDEBAR_ITEMS.map((item) => (
            <Link key={item.href} to={item.href}>
              <motion.div className="flex items-center p-4 text-sm font-medium rounded-lg hover:bg-[#5d695e] transition-colors mb-2">
                <item.icon
                  size={20}
